...
from DeepMTP.main import TwoBranchDotProductModel
...
class DeepMTP:  # pragma: no cover
    ...
    def __init__(self, config, checkpoint_dir=None):
        ...
        if checkpoint_dir is not None:
            if not os.path.isfile(checkpoint_dir):
                raise AttributeError(
                    "The file directory: " + checkpoint_dir + " does not exist!!!"
                )
            else:
        ...
        self.device = torch.device(
            ...
            if (torch.cuda.is_available() and "cuda" in self.config["compute_mode"])
            else "cpu"
        )
        ...
        if self.config["experiment_name"] is None:
            self.experiment_dir = self.config["results_path"] + datetime.now().strftime(
                "%d_%m_%Y__%H_%M_%S"
            )
        else:
            self.experiment_dir = (
                self.config["results_path"] + self.config["experiment_name"]
            )
        ...
        if self.config["general_architecture_version"] == "dot_product":
            ...
            instance_branch_output_dim = (
                self.config["instance_branch_nodes_per_layer"][-1]
                if isinstance(self.config["instance_branch_nodes_per_layer"], list)
                else self.config["instance_branch_nodes_per_layer"]
            )
            target_branch_output_dim = (
                self.config["target_branch_nodes_per_layer"][-1]
                if isinstance(self.config["target_branch_nodes_per_layer"], list)
                else self.config["target_branch_nodes_per_layer"]
            )

        # define models that will be used in the two branches
        if self.config["instance_branch_architecture"] == "MLP":
            self.instance_branch_model = MLP(
                self.config,
                self.config["instance_branch_input_dim"],
                instance_branch_output_dim,
                self.config["instance_branch_nodes_per_layer"],
                self.config["instance_branch_layers"],
                self.config["dropout_rate"],
                self.config["batch_norm"],
            )
        elif self.config["instance_branch_architecture"] == "CONV":
            self.instance_branch_model = ConvNet(
                self.config,
                self.config["instance_branch_input_dim"],
                instance_branch_output_dim,
                self.config["instance_branch_conv_architecture"],
                self.config["instance_branch_conv_architecture_version"],
                self.config["instance_branch_conv_architecture_last_layer_trained"],
                self.config["instance_branch_conv_architecture_dense_layers"],
            )

        if self.config["target_branch_architecture"] == "MLP":
            self.target_branch_model = MLP(
                self.config,
                self.config["target_branch_input_dim"],
                target_branch_output_dim,
                self.config["target_branch_nodes_per_layer"],
                self.config["target_branch_layers"],
                self.config["dropout_rate"],
                self.config["batch_norm"],
            )
        elif self.config["target_branch_architecture"] == "CONV":
            self.target_branch_model = ConvNet(
                self.config,
                self.config["target_branch_input_dim"],
                target_branch_output_dim,
                self.config["target_branch_conv_architecture"],
                self.config["target_branch_conv_architecture_version"],
        ...
        if self.config["general_architecture_version"] == "dot_product":
            self.deepMTP_model = TwoBranchDotProductModel(
                self.config, self.instance_branch_model, self.target_branch_model
            )
        else:
            self.deepMTP_model = TwoBranchMLPModel(
                self.config, self.instance_branch_model, self.target_branch_model
            )
        ...
        if self.config["problem_mode"] == "classification":
            ...
            raise AttributeError(
                "Invalid problem model: " + str(self.config["problem_mode"])
            )
        ...
        self.early_stopping = EarlyStopping(
            ...
            verbose=False,
            metric_to_track=self.config["metric_to_optimize_early_stopping"]
            if self.config["use_early_stopping"]
            else self.config["metric_to_optimize_best_epoch_selection"],
        )
        ...
        with torch.no_grad():
            ...
            for batch_id, batch in enumerate(dataloader):
                ...
                if (
                    (mode == "test")
                    or (
                        (epoch % self.config["eval_every_n_epochs"] == 0)
                        and (self.config["evaluate_val"])
                    )
                    or (self.config["metric_to_optimize_early_stopping"] != "loss")
                    or (self.config["num_epochs"] - 1 == epoch)
                ):
            ...
            if return_predictions:
                return results, pd.DataFrame(
                    {
                        "instance_id": instance_ids_arr,
                        "target_id": target_ids_arr,
                        "true_values": true_scores_arr,
                        "predicted_values": pred_scores_arr,
                    }
                )
            else:
                return results
    ...
    def train(self, train_data, val_data, test_data, verbose=False):  # pragma: no cover
        ...
        train_dataloader = DataLoader(
            BaseDataset(
                ...
                target_transform=self.config["target_train_transforms"],
            ),
            self.config["train_batchsize"],
            shuffle=True,
        ...
        if val_data is not None:
            val_dataloader = DataLoader(
                BaseDataset(
                    ...
                    val_data["X_instance"],
                    val_data["X_target"],
                    instance_transform=self.config["instance_inference_transforms"],
                    target_transform=self.config["target_inference_transforms"],
                ),
                self.config["val_batchsize"],
                shuffle=False,
                num_workers=self.config["num_workers"],
                pin_memory=True,
            )
        ...
        run_story_header = ["mode", "#epoch", "loss"] + [
            m + "_" + av
            for m in self.config["metrics"]
            for av in self.config["metrics_average"]
        ]
        ...
        if self.config["use_tensorboard_logger"]:
            ...
            for key in self.config.keys():
                self.tensorboard_logger.add_text(key, str(self.config[key]))
        if (
            self.config["wandb_project_name"] is not None
            and self.config["wandb_project_entity"] is not None
        ):
            self.wandb_run = wandb.init(
                project=self.config["wandb_project_name"],
                entity=self.config["wandb_project_entity"],
                reinit=True,
            )
        ...
        for epoch in range(self.config["num_epochs"]):
            ...
            for batch_id, batch in enumerate(train_dataloader):
                ...
                if self.config["problem_mode"] == "classification":
                    loss = self.criterion(
                        torch.squeeze(self.act(pred_score), 1), true_score
                    )
                else:
            ...
            if (self.config["evaluate_train"]) and (
                (epoch % self.config["eval_every_n_epochs"] == 0)
                or (self.config["num_epochs"] - 1 == epoch)
            ):
            ...
            train_run_story.append(
                ["train", str(epoch), round(mean_loss, 4)]
                + [
                    round(train_results["train_" + m + "_" + av], 4)
                    if "train_" + m + "_" + av in train_results
                    else "-"
                    for m in self.config["metrics"]
                    for av in self.config["metrics_average"]
                ]
            )
            temp_train_progress_df = pd.DataFrame(
                np.array(train_run_story).reshape(1, -1), columns=run_story_header
            )
            train_progress_table.add_rows(temp_train_progress_df)

            # update the loggers
            results_to_log = {
                "train_" + m + "_" + av: train_results["train_" + m + "_" + av]
                for m in self.config["metrics"]
                for av in self.config["metrics_average"]
                if "train_" + m + "_" + av in train_results
            }
            ...
            if self.tensorboard_logger is not None:
                for k, v in results_to_log.items():
                    self.tensorboard_logger.add_scalar(k.replace("_", "/"), v, epoch)
            ...
            val_run_story.append(
                ["val", str(epoch), round(val_results["val_loss"], 4)]
                + [
                    round(val_results["val_" + m + "_" + av], 4)
                    if "val_" + m + "_" + av in val_results
                    else "-"
                    for m in self.config["metrics"]
                    for av in self.config["metrics_average"]
                ]
            ...
            results_to_log.update(
                {
                    "val_" + m + "_" + av: val_results["val_" + m + "_" + av]
                    for m in self.config["metrics"]
                    for av in self.config["metrics_average"]
                    if "val_" + m + "_" + av in val_results
                }
            ...
            if (
                self.early_stopping.early_stop_flag
                and self.config["use_early_stopping"]
            ):
                # print('Early stopping criterion met. Training stopped!!!')
                break
            else:
                val_run_story[-1].append(
                    str(self.early_stopping.counter)
                    + "/"
                    + str(self.early_stopping.patience)
                )
            ...
            temp_val_progress_df = pd.DataFrame(
                np.array(val_run_story[-1]).reshape(1, -1),
                columns=run_story_header + ["early_stopping"],
            )
            val_progress_table.add_rows(temp_val_progress_df)
            # update the streamlit train-val loss plot

            train_val_loss_chart.add_rows(
                pd.DataFrame(
                    np.array([train_run_story[-1][2], val_run_story[-1][2]]).reshape(
                        1, -1
                    ),
                    columns=["train", "val"],
                )
        ...
        val_best_performance_results = (
            self.early_stopping.get_best_performance_results()
        )
        results_to_log = {
            "best_val_"
            + m
            + "_"
            + av: val_best_performance_results["val_" + m + "_" + av]
            for m in self.config["metrics"]
            for av in self.config["metrics_average"]
            if "val_" + m + "_" + av in val_best_performance_results
        }
        if self.wandb_run is not None:
        ...
        test_run_story_table.add_row(
            ["test", self.early_stopping.get_best_epoch(), "-"]
            + [
                round(test_results["test_" + m + "_" + av], 4)
                for m in self.config["metrics"]
                for av in self.config["metrics_average"]
            ]
        )
        ...
        test_progress_df = pd.DataFrame(
            ...
            columns=run_story_header,
        )
        test_progress_table.table(test_progress_df)
        # update the loggers
        results_to_log = {
            "test_" + m + "_" + av: test_results["test_" + m + "_" + av]
            for m in self.config["metrics"]
            for av in self.config["metrics_average"]
        }
        if self.wandb_run is not None:
        ...
        if (
            self.config["instance_train_transforms"] is not None
            and self.config["target_train_transforms"] is not None
        ):
        ...
        final_info.info(
            "You can find the best model as well as other usefull files generated from this experiment in the following directory: "
    ...
    def predict(
        self, data, return_predictions=False, verbose=False
    ):  # pragma: no cover
        self.deepMTP_model.to(self.device)
        dataloader = DataLoader(
            BaseDataset(self.config, data["y"], data["X_instance"], data["X_target"]),
            self.config["val_batchsize"],
            shuffle=False,
            num_workers=self.config["num_workers"],
        )
